...
import { Brain, Menu, Trophy, Star, ChevronLeft, WifiOff, Loader2, Languages, Sparkles, BookOpen, Award, ShieldCheck, Camera } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    return (
      <div className="fixed inset-0 bg-blue-600 flex flex-col items-center justify-center z-[100] text-white">
        ...
        <div className="animate-fade-in text-center">
          ...
          <p className="text-blue-200 text-[10px] font-black uppercase tracking-[0.3em] mt-2">v3.3 Build Stable</p>
        </div>
      </div>
    );
  }
          ...
          <div className="p-4 space-y-6 animate-slide-in pb-32">
            <header className="flex justify-between items-center py-2 px-2">
              <div className="flex items-center gap-3">
                <button onClick={() => { setIsSidebarOpen(true); playClickSound(); }} className="p-3 bg-white rounded-2xl shadow-sm border border-gray-100 text-gray-700 active:scale-90 transition-all">
                  <Menu size={24} />
                </button>
                <div>
              ...
              <button onClick={() => { setLanguage(l => l === Language.EN ? Language.HI : Language.EN); playClickSound(); }} className="bg-blue-600 text-white px-4 py-2 rounded-2xl text-[10px] font-black flex items-center gap-2 shadow-lg shadow-blue-100">
            ...
            <div className="bg-gradient-to-br from-[#2563eb] to-[#4338ca] p-6 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
               <Trophy size={160} className="absolute -right-10 -bottom-10 opacity-10 rotate-12 transition-transform duration-500" />
            ...
            <div className="pt-6 border-t border-gray-100 flex flex-col items-center gap-2">
               <button onClick={() => navigateTo(View.PRIVACY)} className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-blue-600 transition-colors p-3 bg-gray-50 rounded-2xl border border-gray-100">
                 <ShieldCheck size={14} className="text-blue-600" /> View Privacy Policy
               </button>
               <p className="text-[9px] text-gray-300 font-bold uppercase tracking-widest">Master Sahab Suite v3.3</p>
            </div>
          </div>
        );
      case View.SOLVE:
        return (
          <div className="p-4 flex flex-col gap-6 animate-slide-in pb-20 h-full">
            ...
            <div className="bg-white p-5 rounded-[2.5rem] border-2 border-gray-50 shadow-sm flex flex-col gap-4 flex-1">
              <textarea value={solvePrompt} onChange={e => setSolvePrompt(e.target.value)} placeholder={t.solvePrompt} className="w-full flex-1 p-4 rounded-3xl outline-none resize-none bg-gray-50 text-base font-bold transition-all" />
              <button onClick={handleSolve} disabled={isSolverLoading} className="bg-blue-600 text-white p-5 rounded-[1.5rem] font-black shadow-xl shadow-blue-100 disabled:opacity-50 active:scale-[0.98] flex items-center justify-center gap-3 transition-transform">
      ...
      case View.PROFILE:
        return (
          <div className="p-4 flex flex-col gap-6 animate-slide-in">
            ...
            <button onClick={() => navigateTo(View.PRIVACY)} className="w-full flex items-center justify-between p-6 bg-white border-2 border-gray-50 rounded-[2rem] font-black text-gray-700 active:bg-gray-50 transition-all">
              <div className="flex items-center gap-3"><ShieldCheck className="text-blue-600" size={20} /> Privacy & Safety Guide</div>
              <ChevronLeft className="rotate-180 text-gray-300" size={18} />
            </button>
            <button onClick={() => { playClickSound(); authService.logout(); setUser(null); }} className="w-full p-6 bg-red-50 text-red-600 font-black rounded-[2rem] border-2 border-red-100 active:bg-red-100 transition-all">Logout Account</button>
          </div>
        );
  ...
  return (
    <div className="max-w-md mx-auto h-screen bg-[#FAFAFA] relative overflow-hidden flex flex-col">
      <Sidebar user={user} isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} onNavigate={navigateTo} onLogout={() => { playClickSound(); authService.logout(); setUser(null); }} />
      
      <main className="flex-1 overflow-y-auto pt-safe relative">
        {renderViewContent()}
      </main>

      {/* Persistent Full-screen Overlays for Calculator and Privacy to ensure they never go 'blank' */}
      {currentView === View.CALCULATOR && (
        <div className="fixed inset-0 bg-white z-[70] animate-pop">
          <CalculatorView onBack={() => navigateTo(View.HOME)} />
        </div>
      )}
      {currentView === View.PRIVACY && (
        <div className="fixed inset-0 bg-white z-[70] animate-pop">
          <PrivacyView onBack={() => navigateTo(View.HOME)} />
        </div>
      )}
      {currentView === View.CAMERA && (
        <div className="fixed inset-0 bg-black z-[70] animate-fade-in">
          <CameraView language={language} onBack={() => navigateTo(View.HOME)} />
        </div>
      )}

      {/* Tab Navigation - Only visible on core study tabs */}
      {[View.HOME, View.SOLVE, View.PROFILE, View.QUIZ].includes(currentView) && (
        <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-2xl border-t-2 border-gray-50 flex justify-around items-center py-6 z-40 px-6 rounded-t-[3rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          {NAV_ITEMS.map((item) => (
            <button key={item.id} onClick={() => navigateTo(item.id as View)} className={`flex flex-col items-center gap-1.5 transition-all relative ${currentView === item.id ? 'text-blue-600 scale-110' : 'text-gray-300 hover:text-gray-500'}`}>
